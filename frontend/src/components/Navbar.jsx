...
const Navbar = () => {
  ...
  return (
    <div className="flex items-center  justify-around py-6 font-medium bg-slate-950">
      <Link to="/">
        <h1 className="font-bold font-serif text-4xl text-white">STYLEFORGE</h1>
      </Link>
      ...
      <div className="flex items-center gap-10">
        ...
        <Link to="/cart" className="relative">
          ...
          <p className="absolute right-[-5px] bottom-[-5px] w-4 text-center leading-4 bg-white aspect-square rounded-full text-[8px]">
            {getCartCount()}
          </p>
        </Link>
        <TbMenuDeep
          onClick={() => setVisible(true)}
          className="text-white size-7 cursor-pointer sm:hidden"
          alt=""
        />
      </div>
      ...
      >
        <div className="flex flex-col text-gray-600">
          ...
          <NavLink
            onClick={() => setVisible(false)}
            className="z-50 py-2 pl-6 border"
            to="/collection"
          >
            Collection
          </NavLink>
